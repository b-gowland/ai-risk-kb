...
description: "Computer-use agents control a keyboard and mouse on behalf of their user — browsing the web, filling forms, operating applications. Any content the agent sees can contain instructions that redirect its actions. The agent has no way to distinguish a legitimate instruction from a malicious one hidden in a webpage."
---
