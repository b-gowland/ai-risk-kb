...
description: "Malicious instructions hidden in content processed by an AI system can hijack the system to take unauthorised actions — including sending data to attackers or making decisions on their behalf."
---
...
### C2-006 — MCP server trust boundary controls

**Owner:** Technology | **Type:** Preventive | **Effort:** Medium | **Go-live required:** Yes

Model Context Protocol (MCP) servers extend LLM capability by providing tools and data access. A compromised or malicious MCP server represents a direct indirect prompt injection surface: the server can return content containing injected instructions that the agent then acts on with full tool permissions. MITRE ATLAS v5.3 documents data exfiltration via MCP server as a distinct technique.

Controls for MCP deployments: (1) treat MCP server responses as untrusted data — apply the same sandboxing as for any externally-sourced content; (2) vet all MCP servers before integration, particularly open-source servers from unverified publishers; (3) scope MCP server permissions to the minimum required for each specific task; (4) log all MCP tool calls and responses for anomaly detection; (5) implement allowlists of approved MCP servers — reject requests to connect to unlisted servers at the agent orchestration layer.

**Jurisdiction notes:** AU — recommended under ACSC AI Security Guidance | EU — required for high-risk AI systems under EU AI Act Art. 15 robustness requirements | US — NIST Cyber AI Profile IR 8596
...
| MCP server allowlist coverage | 100% of connected MCP servers on approved list | Reviewed on each integration change |
...
**AI agent context poisoning via MCP server (2025):** MITRE ATLAS v5.3 documented a technique where a malicious or compromised MCP server returns responses containing injected instructions — directing the connected LLM agent to exfiltrate data or take unauthorised actions. The agent processes the MCP response as trusted data and executes the embedded instructions using its full tool permissions. This represents a supply-chain variant of indirect prompt injection, where the injection point is the integration layer rather than user-submitted content. Source: MITRE ATLAS v5.3.0 (2025); ATLAS technique AML.T0051.001 (LLM Prompt Injection: Indirect).

**LLM jailbreak techniques documented in ATLAS v5.6 (2025):** MITRE ATLAS v5.6 added structured documentation of LLM jailbreak techniques — adversarial inputs that cause LLMs to override their safety instructions and produce outputs they are configured to refuse. Jailbreak techniques are a category of direct prompt injection targeting model safety guardrails rather than agent tool permissions. Practitioners deploying LLMs with safety-critical constraints should treat jailbreak testing as part of pre-deployment red teaming. Source: MITRE ATLAS v5.6.0 (2025).
