...
<div style={{display:'flex',gap:'0.5rem',flexWrap:'wrap',marginBottom:'1.5rem'}}>
  ...
  <span className="framework-chip">NIST AI 600-1</span> <span className="framework-chip">MITRE ATLAS AML.T0020 Poison Training Data</span> <span className="framework-chip">OWASP ML01</span> <span className="framework-chip">ISO 42001 Cl. 6.3.1</span>
</div>
...
**AI Agent Tool Data Poisoning (ATLAS v5.2–v5.4, 2025):** MITRE ATLAS v5.2 and v5.4 document a technique specific to agentic systems: poisoning the tools and data sources that AI agents rely on rather than the model weights themselves. An agent that reads from a database, retrieves documents, or calls external APIs as part of its workflow can be manipulated if any of those data sources are compromised. The agent processes poisoned tool outputs with the same trust as legitimate ones. This extends the poisoning attack surface beyond the training pipeline to include runtime data dependencies. Source: MITRE ATLAS v5.2.0 and v5.4.0 (2025).

**Publish Poisoned AI Agent Tool (ATLAS v5.4, 2025):** MITRE ATLAS v5.4 documents a supply-chain poisoning technique where an adversary publishes a malicious AI agent tool to a public repository — a plugin, MCP server, or agent extension — that appears legitimate but contains embedded malicious behaviour. Organisations that allow AI agents to install or connect to tools from public repositories without vetting inherit the risk. Source: MITRE ATLAS v5.4.0 (2025).
...
**[▶ Play this scenario in the AI Risk Training Module](https://b-gowland.github.io/ai-risk-training/#/scenario/c1-data-poisoning)** — Training Data Poisoning & Model Integrity, three personas, ~15 minutes.
