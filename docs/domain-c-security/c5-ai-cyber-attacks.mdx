...
AI removes the indicators that previously helped staff identify phishing: poor grammar, generic greetings, suspicious formatting. AI-generated phishing is significantly harder to detect than traditional phishing — often appearing identical to genuine correspondence. AI-assisted vulnerability discovery allows attackers to identify and exploit flaws faster than defenders can patch. The threat level has permanently shifted upward.
...
**Australian university breach 200,000 records (illustrative):** University suffered a breach after receiving AI-generated phishing emails that precisely mimicked official HR communications — with no grammar or formatting cues to identify them as phishing. Detection required metadata analysis rather than content review.
...
**AI-generated malicious command generation (ATLAS v5.2, 2025):** MITRE ATLAS v5.2 documents Generate Malicious Commands as a distinct AI-enabled technique — using LLMs to produce shell commands, scripts, or API calls designed to exploit vulnerabilities or escalate privileges. LLMs can generate targeted attack code tailored to a specific environment when provided with reconnaissance data, substantially lowering the expertise barrier for post-compromise lateral movement. Source: MITRE ATLAS v5.2.0 (2025).

**Machine Compromise via AI-enabled techniques (ATLAS v5.5, 2025):** MITRE ATLAS v5.5 documents Machine Compromise as a distinct technique where AI capabilities are used to identify and exploit vulnerabilities in target systems at a scale and speed not achievable manually — including OS credential dumping (ATLAS v5.1) and exploitation for credential access (ATLAS v5.4). Organisations relying on patch cycle timelines as their primary vulnerability management control face a compressed window when AI-assisted exploitation is in use. Sources: MITRE ATLAS v5.1.0, v5.4.0, v5.5.0 (2025).
...
**[▶ Play this scenario in the AI Risk Training Module](https://b-gowland.github.io/ai-risk-training/#/scenario/c5-ai-cyber-attacks)** — AI-Enabled Business Email Compromise, four personas, ~14 minutes.
