...
Each entry includes persona-specific hooks in a tabbed interface. All entries include three practitioner tabs. Entries linked from an At Home scenario (A1, C4, E1) also include an **At home** tab written in plain language for general readers.
...
<Tabs>
  ...
  {/* At home tab — only on A1, C4, E1 (entries linked to an At Home scenario) */}
  <TabItem value="everyday" label="At home">
  [Plain language for general public. No jargon, no framework codes.
   Ends with a link back to the scenario. See CONTENT_STYLE_GUIDE for voice rules.]
  </TabItem>
</Tabs>
```
